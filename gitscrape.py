...
base_url = "https://www.bcliquorstores.com/ajax/browse?category=???&sort=name.raw:asc&size=5000"

categories = ["wine", "beer", "spirits", "coolers"]

for category in categories:

    items = requests.get(base_url.replace("???", category)).json()["hits"]["hits"]
    ...
    df.to_csv(f"bcliquordata-{category}.csv")
    df.to_json(f"bcliquordata-{category}.json", orient='records', lines=True)
